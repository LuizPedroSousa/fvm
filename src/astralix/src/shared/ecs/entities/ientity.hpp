...
#define ENTITY_INIT_DTO(dto) Entity(dto.id, dto.family_id, dto.name)
  ...
  public:
    ...
    std::vector<IComponent*> get_components() const {
      ...
      for (auto entity_component = entity_components_table.begin(),
        ...
        auto component =
          manager->m_component_table.find(entity_component->second);

        if (component == manager->m_component_table.end() || component->second == nullptr) {
          continue;
        }

        components.push_back(component->second.get());
      }
