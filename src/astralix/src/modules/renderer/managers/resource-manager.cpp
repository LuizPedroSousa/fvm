...
Texture *ResourceManager::load_texture(ResourceID id, std::string name,
                                       int color_type, const char *filename) {
  ...
  auto texture_created = Texture::create(id, name, color_type, filename);

  ASSERT_COMPARE_THROW(texture_created);

  auto texture_ptr = std::make_unique<Texture>(texture_created.right());
...
Shader *ResourceManager::load_shader(ResourceID id, const char *vertex_filename,
  ...
  auto shader_ptr = std::make_unique<Shader>(created_shader.right());
...
Material *ResourceManager::load_material(ResourceID material_id,
  ...
  auto get_texture = [&](aiTextureType type) {
    ...
    for (unsigned int i = 0; i < ai_material->GetTextureCount(type); i++) {
      ...
      auto texture_ptr =
          load_texture(texture_id, get_name(), GL_RGBA,
                       ("models/" + std::string(filename)).c_str());
  ...
  auto material_ptr = std::make_unique<Material>(created_material);
...
Model *ResourceManager::load_model(ResourceID id, const char *filename) {
  ...
  std::unique_ptr<Model> model_ptr = std::make_unique<Model>(model.right());
