...
void OpenGLShader::attach() const {
  ASTRA_ASSERT_THROW(m_renderer_id == -1, "Shader not found");
...
uint32_t OpenGLShader::compile(const std::string &path, uint32_t type) {
  ...
  if (!success) {
    ...
    glGetShaderInfoLog(shader_id, sizeof(shader_error), NULL, shader_error);

    std::string error = "Can't load shader " + std::string(path) + "\n" +
                        std::string(shader_error);

    ASTRA_ASSERT_THROW(true, error);
  };
