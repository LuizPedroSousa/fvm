...
vec3 calculate_directional_light(DirectionalLight light, vec3 normal, vec2 exture_coordinates);
vec3 point_light(PointLight light, vec3 view_direction, vec3 normal, vec2 texture_coordinates);
...
float get_shadow(vec3 light_direction, vec3 normal);
...
void main() {
    ...
    result += calculate_directional_light(directional_light, normal, texture_coordinates);

    for (int i = 0; i < 4; ++i)
    {
        result += point_light(point_lights[i], view_direction, normal, texture_coordinates);
    }
...
vec3 calculate_directional_light(DirectionalLight light, vec3 normal, vec2 texture_coordinates) {
    ...
    vec3 specular =
        light.exposure.specular * get_specular(light_direction, view_direction, normal);

    float shadow = get_shadow(light_direction, normal);

    return (ambient + shadow * (diffuse + specular)) * color;
};

vec3 point_light(PointLight light, vec3 view_direction, vec3 normal, vec2 texture_coordinates) {
    vec3 direction = normalize(light.position - obj_coordinates.tangent_fragment);

    vec3 color = texture(materials[0].diffuse, texture_coordinates).rgb;

    float distance = length(light.position - obj_coordinates.tangent_fragment);
    ...
    vec3 specular =
        light.exposure.specular * get_specular(direction, view_direction, normal);
