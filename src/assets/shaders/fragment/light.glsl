...
vec3 calc_directional_light(DirectionalLight light,vec3 normal){
  ...
  float spec=pow(max(dot(view_direction,reflect_direction),0.),materials[0].shininess);
  
  vec3 specular=light.exposure.specular*spec;
  
  for(int i=0;i<NR_MATERIALS;i++){
    vec3 diffuse_texture=vec3(texture(materials[i].diffuse,_texture_cord));
    vec3 specular_texture=vec3(texture(materials[i].specular,_texture_cord));
...
vec3 calc_point_light(PointLight light,vec3 normal,vec3 view_direction)
{
  vec3 direction=normalize(light.position-_fragment_position);
  float distance=length(light.position-_fragment_position);
  float attenuation=1./(light.attenuation.constant+light.attenuation.linear*distance+light.attenuation.quadratic*(distance*distance));
  ...
  vec3 dir=normalize(light.position-_fragment_position);
  
  float spec=pow(max(dot(dir,reflect_direction),0.),materials[0].shininess);
  vec3 specular=light.exposure.specular*spec;
...
vec3 calc_spot_light(SpotLight light,vec3 normal,vec3 view_direction){
  ...
  float smooth_intensity=clamp((theta-light.outer_cut_off)/epsilon,0.,1.);
