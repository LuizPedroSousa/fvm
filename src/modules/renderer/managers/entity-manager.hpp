...
template <typename T> class EntityContainer : public IEntityContainer {
  using TypedEntityContainer =
      std::unordered_map<EntityID, std::shared_ptr<Entity<T>>>;
...
public:
  EntityContainer() {}
  TypedEntityContainer &get_typed_entity_container() {
    return m_typed_container;
  }
...
public:
  ...
  template <typename T, typename... Args> T &add_entity(Args &&...params) {
    ...
    auto entity_ptr = std::make_shared<T>(id, this->m_component_manager,
    ...
    auto container =
        container_ptr.right()->get_typed_entity_container().try_emplace(
            id, entity_ptr);

    ASSERT_THROW(!container.second,
                 "Error inserting Entity in typed containers !");
...
private:
  ...
  Either<BaseException, EntityContainer<T> *> get_entity_container() {
    ...
    auto entity_container = this->m_entity_typed_table.try_emplace(
        type_id, std::make_unique<EntityContainer<T>>());

    ASSERT(!entity_container.second, "Failed to create EntityContainer<T>!");

    return reinterpret_cast<EntityContainer<T> *>(
        entity_container.first->second.get());
  }
