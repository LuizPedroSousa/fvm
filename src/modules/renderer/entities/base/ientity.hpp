...
#define ENTITY_INIT_PARAMS const EntityID &id, ComponentManager *component_manager
...
class IEntity {
  ...
  public:
  IEntity(ENTITY_INIT_PARAMS) : m_component_manager(component_manager), m_entity_id(id), m_active(true) {}
  ...
  T *add_component(Args &&...params) {
    return m_component_manager->add_component<T>(m_entity_id, std::forward<Args>(params)...);
  }
  ...
  inline bool operator==(const IEntity &other) const { return m_entity_id == other.m_entity_id; }
  inline bool operator!=(const IEntity &other) const { return m_entity_id != other.m_entity_id; }
  inline bool operator==(const IEntity *other) const { return m_entity_id == other->m_entity_id; }
  inline bool operator!=(const IEntity *other) const { return m_entity_id != other->m_entity_id; }
};