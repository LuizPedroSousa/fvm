...
#define ASSERT(EXPRESSION, MESSAGE) \
  if (EXPRESSION) return BaseException(__FILE__, __LINE__, MESSAGE)
...
#define ASSERT_COMPARE(EXPRESSION) \
  if (EXPRESSION.isLeft()) return EXPRESSION.left()

#define ASSERT_COMPARE_THROW(EXPRESSION) \
  if (EXPRESSION.isLeft()) throw EXPRESSION.left()
...
class Either {
  ...
  public:
  Either(L exception)
      : left_value(exception), has_left(true) {
  }
  ...
  Either(R result)
      : right_value(result), has_left(false) {
  }
